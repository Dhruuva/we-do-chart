(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["WeChart"] = factory();
	else
		root["WeChart"] = factory();
...
"use strict";
/* harmony import */ var _node_modules_mini_css_extract_plugin_dist_loader_js_ref_11_oneOf_1_0_node_modules_css_loader_dist_cjs_js_ref_11_oneOf_1_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_11_oneOf_1_2_node_modules_stylus_loader_index_js_ref_11_oneOf_1_3_node_modules_cache_loader_dist_cjs_js_ref_0_0_node_modules_vue_loader_lib_index_js_vue_loader_options_weChart_vue_vue_type_style_index_0_id_70a9b30a_scoped_true_lang_stylus___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__("08d1");
/* harmony import */ var _node_modules_mini_css_extract_plugin_dist_loader_js_ref_11_oneOf_1_0_node_modules_css_loader_dist_cjs_js_ref_11_oneOf_1_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_11_oneOf_1_2_node_modules_stylus_loader_index_js_ref_11_oneOf_1_3_node_modules_cache_loader_dist_cjs_js_ref_0_0_node_modules_vue_loader_lib_index_js_vue_loader_options_weChart_vue_vue_type_style_index_0_id_70a9b30a_scoped_true_lang_stylus___WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_node_modules_mini_css_extract_plugin_dist_loader_js_ref_11_oneOf_1_0_node_modules_css_loader_dist_cjs_js_ref_11_oneOf_1_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_11_oneOf_1_2_node_modules_stylus_loader_index_js_ref_11_oneOf_1_3_node_modules_cache_loader_dist_cjs_js_ref_0_0_node_modules_vue_loader_lib_index_js_vue_loader_options_weChart_vue_vue_type_style_index_0_id_70a9b30a_scoped_true_lang_stylus___WEBPACK_IMPORTED_MODULE_0__);
...
function DoAxes() {
  ...
  var yScale = function yScale(hi, lo, tky, a) {
    var md = Math.abs(hi + lo) / 2;
    var mv = _this.movMin;
    var nn = mv.toString().indexOf(".") > 0 ? mv.toString().length - mv.toString().indexOf(".") - 1 : 0; // console.log(nn,'#############decimal place', Math.ceil(mv * 1000000)/1000000 )
    //let d =  Math.round((hi-lo)/tky)

    var d = (hi - lo) / tky < tky * mv ? tky * mv : (hi - lo) / tky;
    if (d >= 1) d = Math.round((hi - lo) / tky);
    var step = (a.y.y1 - a.y.y2) / tky,
        f = a.y.y2;
    var low = Math.ceil(hi / d) * d,
        high = low,
        arr = [{
      id: tky,
      price: Number(high).toFixed(nn),
      y: f
    }],
        t = high;
    ...
    var stp = (high - low) / tky;

    for (var i = tky - 1; i >= 0; i--) {
      var p = Number(t -= stp).toFixed(nn);
      f += step;
      arr.push({
        id: i,
        price: p,
    ...
    var rtn = {
      y: arr,
      high: Number(high).toFixed(nn),
      low: Number(low).toFixed(nn)
    }; // console.log(" low ", low," high ", high," hi ", hi , " lo " ,lo, " stp ", stp, " d ", d) ;

    return rtn;
  };

  var minMove = function minMove(ds) {
    var mMov = +Infinity,
        arr = ds.map(function (a) {
      return a.price;
    }),
        mxl = Math.max.apply(Math, _toConsumableArray(arr.map(function (a) {
      return ('' + a).length;
    })));

    for (var i = 0; i <= arr.length - 1; i++) {
      var z = i == 0 ? arr[i] : Math.abs(arr[i] - arr[i - 1]);
      mMov = mMov > z ? z : mMov; //	mxl =( (''+mMov).length > mxl) ? (''+mMov).length:mxl
    } //this.movMin=Math.ceil(mMov * 100)/100;
    //this.movMin=Math.round(mMov);


    _this.movMin = mMov; //console.log(mxl,'>>>>>>>>>this.movMin',this.movMin)
  ...
  var formulaY = function formulaY(data, off, tky, wbx, minMove, format) {
    _this.movMin = minMove;

    var proxy = _toConsumableArray(data); //console.log ("--- off++++ ",off);
    ...
    ds.reverse();
    var mini = Math.min.apply(Math, _toConsumableArray(ds.map(function (a) {
      return a.price;
    })));
    var maxi = Math.max.apply(null, ds.map(function (a) {
      return a.price;
    }));
    var pPct = (off.y.y1 - off.y.y2) / 100;
    var scale = yScale(maxi, mini, tky, off);
    var hl = scale.high - scale.low;

    while (scale.y.length > 0) {
      ticksY.push(scale.y.pop());
    } //ticksY.reverse()
    //console.log(ticksY)
    // For  AxisX  & xy calc .........................


    var w = off.x.x2 - off.x.x1;
    var stepx = w / (ds.length - 1);
    var xtm = off.x.x1,
        nxLbl = xtm;
    var wbox = wbx * 1.5;
    var skip = (ds.length - 1) * wbox / ((ds.length - 1) * stepx),
        n = 1; //console.log("skip ", ds[0])
    ...
    ds.forEach(function (a, i) {
      ...
      var tms = a.tm; //let tms= (format)? dateFormat( a.tm,format):a.tm
      ...
      if (i == 0) {
        ...
        if (format) {
          _rowx.tm = dateFormat(tms, format); // this for x labels

          _rowx.d = new Date(tms).getDate();
          _rowx.mm = dateFormat(tms, 'mmm');
          _rowx.yy = dateFormat(tms, 'yyyy');
        }

        ticksX.push(_rowx); //console.log(nxLbl, "nxLbl  ", rowx , " wbx ",wbx )

        nxLbl += wbox;
        n = 1;
      }

      n++; //console.log("row pointYX ", row )
    });
  };
  ...
  var clear = function clear() {
    ...
    while (ticksX.length > 0) {
      ticksX.pop();
    }

    while (ticksY.length > 0) {
      ticksY.pop();
    }

    while (pointY.length > 0) {
      pointY.pop();
    }

    while (pointX.length > 0) {
      pointX.pop();
    }
  ...
  var oddTickSize = function oddTickSize(points, mayBeSize) {
    var rtn = points / 24;

    for (var i = mayBeSize; i <= 24; i++) {
      var remaider = points % i;
  ...
  var fillAxisY = function fillAxisY(data, height, ofy) {
    var proxy = _toConsumableArray(data);
    ...
    var priceOffset = (maxi - mini) / 100 * 5;
    mini -= priceOffset;
    maxi += priceOffset;
    var y0 = ofy,
        y1 = height - ofy; //let step = (y1-y0)/(maxi-mini);

    var step = (y1 - y0) / ((maxi - mini) * 100); //let step1 = (maxi-mini)/(y1-y0);
    ...
    ds.forEach(function (a, i) {
      ...
      pointY.push(row); // let f=ticksY.find(b=>b.price <= a.price)[0]
  ...
  var fillAxisX = function fillAxisX(off) {
    var w = off.x.x2 - off.x.x1;
    var step = w / (ds.length + 2);
    var mini = Math.min.apply(Math, _toConsumableArray(ds.map(function (a) {
      return new Date(a.tm).getTime();
    }))) - 1 * 24 * 60 * 60 * 1000;
    var maxi = Math.max.apply(null, ds.map(function (a) {
      return new Date(a.tm).getTime();
    })) + 1 * 24 * 60 * 60 * 1000;
    var n = off.x.x1,
        id = 1,
        d = new Date(mini).toISOString().slice(0, 10);
    ...
    pointY.forEach(function (a) {
      var f = pointX.find(function (b) {
        return b.id == a.id;
      });
      var row = a;
      row.dtm = f.dtm;
      row.x = f.x; // console.log(row);

      pointYX.push(row);
    });
    return console.log(" fillAxisX- ", new Date(mini).toISOString().slice(0, 10), new Date(maxi).toISOString().slice(0, 10));
  }; // dev
  ...
  this.formulaY = function (data, off, tky, wbx, minMove, format) {
    return formulaY(data, off, tky, wbx, minMove, format);
  };

  this.minMove = function (data) {
    return minMove(data);
  };

  this.fillAxisY = function (data, height, ofy, wbx) {
    return fillAxisY(data, height, ofy, wbx);
  };

  this.fillAxisX = function (off) {
    return fillAxisX(off);
  };

  this.ticksY = function () {
    return ticksY;
  };
  ...
  this.ticksX = function () {
    return ticksX;
  };

  this.pointX = function () {
    return pointX;
  };

  this.pointYX = function () {
    return pointYX;
  };

  this.yScale = function (hi, lo, tky, yy) {
    return yScale(hi, lo, tky, yy);
  };
...
// CONCATENATED MODULE: ./node_modules/cache-loader/dist/cjs.js?{"cacheDirectory":"node_modules/.cache/vue-loader","cacheIdentifier":"03440b3c-vue-loader-template"}!./node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!./node_modules/pug-plain-loader!./node_modules/cache-loader/dist/cjs.js??ref--0-0!./node_modules/vue-loader/lib??vue-loader-options!./src/components/weChart.vue?vue&type=template&id=70a9b30a&scoped=true&lang=pug&
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('svg',{ref:"sheet",attrs:{"id":"sheet","viewBox":_vm.viewBoxSet,"xmlns":"http://www.w3.org/2000/svg"},on:{"mousedown":_vm.startDrag,"mousemove":_vm.mousemove}},[_c('defs',[_c('pattern',{attrs:{"id":"smallGrid","width":"10","height":"10","patternunits":"userSpaceOnUse"}},[_c('path',{attrs:{"d":"M 10 0 L 0 0 0 10","fill":"none","stroke":"gray","stroke-width":"1.5"}})]),_c('pattern',{attrs:{"id":"grid","width":"100","height":"100","patternunits":"userSpaceOnUse"}},[_c('rect',{attrs:{"width":"100","height":"100","fill":"url(#smallGrid)"}},[_c('path',{attrs:{"d":"M 100 0 L 0 0 0 100","fill":"none","stroke":"gray","stroke-width":"1"}})])])]),_c('rect',{ref:"chartSheet",staticClass:"chartSheet",attrs:{"x":"0","y":"0","width":_vm.ds.width,"height":_vm.ds.height}}),_c('circle',{staticClass:"titlesDot",attrs:{"cx":_vm.axis.x.x1,"cy":_vm.axis.y.y2-1-_vm.fs/3,"r":_vm.fs/3}}),_c('text',{ref:"titles",staticClass:"legend",attrs:{"id":"legend","x":_vm.axis.x.x1+1+_vm.fs/3,"y":_vm.axis.y.y2-1,"font-size":_vm.fs}},[_vm._v(_vm._s(_vm.dPoint.txt))]),_c('text',{staticClass:"titles",attrs:{"id":"title","x":_vm.axis.x.x1+(_vm.axis.x.x2-_vm.axis.x.x1)/2+1+_vm.fs/3,"y":_vm.axis.y.y2-1,"font-size":_vm.fs*1.2}},[_vm._v(_vm._s(_vm.title)+" ")]),_c('line',{staticClass:"axisY",attrs:{"x1":_vm.axis.y.x1,"x2":_vm.axis.y.x2,"y1":_vm.axis.y.y1,"y2":_vm.axis.y.y2}}),_c('line',{staticClass:"axisX",attrs:{"x1":_vm.axis.x.x1,"x2":_vm.axis.x.x2,"y1":_vm.axis.x.y1,"y2":_vm.axis.x.y2}}),(!_vm.cross.hide)?_c('g',[_c('line',{staticClass:"cross",attrs:{"x1":_vm.cross.v.x1,"x2":_vm.cross.v.x2,"y1":_vm.cross.v.y1,"y2":_vm.cross.v.y2}}),_c('line',{staticClass:"cross",attrs:{"x1":_vm.cross.h.x1,"x2":_vm.cross.h.x2,"y1":_vm.cross.h.y1,"y2":_vm.cross.h.y2}})]):_vm._e(),_vm._l((_vm.ticksY),function(r){return _c('g',{key:r.y,staticClass:"ticksY"},[(_vm.showGrid)?_c('line',{staticClass:"grids",attrs:{"x1":_vm.axis.x.x1,"x2":_vm.axis.x.x2,"y1":r.y,"y2":r.y}}):_vm._e(),_c('line',{staticClass:"ticks",attrs:{"x1":_vm.axis.y.x1,"x2":_vm.axis.y.x1+_vm.tsz.size,"y1":r.y,"y2":r.y}}),_c('text',{staticClass:"axislabely",attrs:{"x":_vm.axis.y.x1+_vm.tsz.off,"y":r.y+_vm.fs/3,"font-size":_vm.fs}},[_vm._v(_vm._s(r.price))])])}),_vm._l((_vm.ticksX),function(n){return _c('g',{key:n.id,staticClass:"ticksX"},[(_vm.showGrid)?_c('line',{staticClass:"grids",attrs:{"x1":n.x,"x2":n.x,"y1":_vm.axis.y.y1,"y2":_vm.axis.y.y2}}):_vm._e(),_c('line',{staticClass:"ticks",attrs:{"x1":n.x,"x2":n.x,"y1":_vm.axis.x.y1,"y2":_vm.axis.x.y1+_vm.tsz.size}}),_c('text',{staticClass:"axislabelx",attrs:{"x":n.x,"y":_vm.axis.x.y1+_vm.tsz.size+_vm.tsz.off+_vm.fs/2,"font-size":_vm.fs}},[_vm._v(_vm._s(n.tm))])])}),_c('polyline',{staticClass:"plot",attrs:{"points":_vm.pointsAsPolyline}}),_c('g',{staticClass:"leftThumb",attrs:{"transform":'translate('+_vm.thumbY+') scale('+_vm.scl+')'},on:{"mouseleave":function($event){return _vm.stopThumb()},"mouseenter":function($event){_vm.leftDrug = true}}},[_c('polyline',{attrs:{"points":"10,40 0,30 0,10 10,0 31,0 31,40 "}})]),_c('line',{staticClass:"wline",attrs:{"x1":_vm.wline.left.x1,"x2":_vm.wline.left.x2,"y1":_vm.wline.left.y1,"y2":_vm.wline.left.y2,"stroke-width":_vm.wline.left.sSize},on:{"click":_vm.wlineLeftClick}}),_c('rect',{staticClass:"mbody",attrs:{"x":_vm.wline.middle.x,"y":_vm.wline.middle.y,"width":_vm.wline.middle.w,"height":_vm.wline.middle.h},on:{"mouseleave":function($event){_vm.draggingCenter =!_vm.draggingCenter},"mouseenter":function($event){_vm.draggingCenter =!_vm.draggingCenter}}}),_c('g',{staticClass:"rightThumb",attrs:{"transform":'translate('+_vm.thumbYY+') scale('+_vm.scl+')'},on:{"mouseleave":function($event){return _vm.stopThumb()},"mouseenter":function($event){_vm.rightDrug = true}}},[_c('polyline',{attrs:{"points":"0,0 0,40 21,40 30,30 30,10 21,0 "}})]),_c('line',{staticClass:"wline",attrs:{"x1":_vm.wline.right.x1,"x2":_vm.wline.right.x2,"y1":_vm.wline.right.y1,"y2":_vm.wline.right.y2,"stroke-width":_vm.wline.right.sSize},on:{"click":_vm.wlineRightClick}})],2)}
var staticRenderFns = []


// CONCATENATED MODULE: ./src/components/weChart.vue?vue&type=template&id=70a9b30a&scoped=true&lang=pug&
...
// CONCATENATED MODULE: ./src/components/mouseLocation.js
  ...
  methods: {
    ...
    startDrag: function startDrag(evt) {
      ...
      if (this.rightDrug) {
        ...
        var _cursorPt = _pt.matrixTransform(this.svg.getScreenCTM().inverse());

        this.thumbs.right.off = Math.floor(_cursorPt.x) - this.thumbs.right.x; // console.log("startDrag this.thumbs.right.off ", Math.floor(cursorPt.x) - this.thumbs.right.x);
      ...
      if (this.draggingCenter) {
        ...
        var _pt2 = this.svg.createSVGPoint();

        _pt2.x = evt.clientX;
        _pt2.y = evt.clientY;

        var _cursorPt2 = _pt2.matrixTransform(this.svg.getScreenCTM().inverse());

        this.thumbs.right.mp = this.thumbs.right.x - Math.floor(_cursorPt2.x);
        this.thumbs.left.mp = Math.floor(_cursorPt2.x) - this.thumbs.left.x; // this.rightDrug = true;
        // this.leftDrug = true;
        //console.log("  this.draggingCenter ", this.draggingCenter, "this.rightDrug ",this.rightDrug)
      }
    },
    wlineLeftClick: function wlineLeftClick(evt) {
      evt.preventDefault();
      var pt = this.svg.createSVGPoint();
      pt.x = evt.clientX;
      pt.y = evt.clientY;
...
// CONCATENATED MODULE: ./src/components/observeApi.js


/* eslint-disable */
/* harmony default export */ var observeApi = ({
  methods: {
    initObserver: function initObserver() {
      var elem = document.getElementsByClassName(this.parentClass)[0],
          config = {
        attributes: true
      },
          vm = this; // create the observer

      var x = document.getElementById("sheet").parentElement;
      elem = this.svg.parentElement; //console.log('parentElement ',x.class);

      this.ds.width = elem.clientWidth;
      this.ds.height = elem.clientHeight; //console.log('elem.getBoundingClientRect()','initObserver ', elem);
      ...
      var observer = new MutationObserver(function (mutations) {
        ...
        mutations.forEach(function (mutation) {
          ...
          if (mutation.type === "attributes") {
            ...
            height = elem.clientHeight; //	console.log(' elem.style.width= ' , width, ' elem.style.height= ' , height);
            ...
            vm.ds.width = width ? width : vm.ds.width; //console.log("###vm.ds", width, height ,'elemBox',elemBox.width,elemBox.height);
...
// EXTERNAL MODULE: ./src/components/doAxis.js
var components_doAxis = __webpack_require__("e561");
var doAxis_default = /*#__PURE__*/__webpack_require__.n(components_doAxis);
...
/* harmony default export */ var weChartvue_type_script_lang_js_ = ({
  ...
  props: {
    ...
    off: {
      type: [Number, String],
      // Axis offset in percent of height
      default: function _default() {
        return 2;
      }
    },
    ...
    tsz: {
      ...
      } // the label ticks options off should be greater off
  ...
  data: function data() {
    return {
      ...
      // data in  display
      limitSize: 0,
      // data count it's use in limit calculation
      farPoints: [],
      // dev
      move: "don't",
      cross: {
  ...
  mounted: function mounted() {
    ...
    this.leftDrug = true;
    this.draggingLeft = true; //let doAxis = new DoAxis();

    this.initObserver();
    this.xmapData();
    this.getDisplayData(this.axis.x.x1, this.axis.x.x2);
    this.initSlider(); //console.log( " display >>>",this.getDisplayData(400/1.1, 760.80*1.01) )

    console.log("chartSheet>>>>>>>>>>>>  ", this.$refs.chartSheet.style.stroke); //this.getStyle(document.getElementsByClassName("axislabelx")[0])
    //	this.getStyle(this.$refs.chartSheet)

    this.leftDrug = false;
    this.draggingLeft = false;
    this.$nextTick(function () {
      console.log("chartSheet---------  ", _this.$refs.chartSheet.style.stroke);
    });
  },
  watch: {
    showGrid: function showGrid(val) {
    ...
    decimals: function decimals(val) {
      ...
      this.loadChart(); //this.initSlider()
    },
    off: function off(val) {
      console.log("watch off ", val);
      this.loadChart(); //this.initSlider()
    },
    tky: function tky(val) {
      console.log("watch tky ", val);
      this.loadChart(); //this.initSlider()
    },
    scl: function scl(val) {
      console.log("watch scl ", val);
      this.loadChart(); //this.initSlider()
    },
    timeFotmat: function timeFotmat(val) {
  ...
  computed: {
    thumbY: function thumbY() {
      ...
      if (this.draggingCenter && !this.rightDrug && !this.leftDrug && this.moveDrug) {
        ...
        var offset = this.thumbs.left.off ? this.thumbs.left.off : this.scl * 31; //console.log("thumbY x  ",this.pos.x ," this.thumbs.left.x  " ,this.thumbs.left.x, " off ",offset);

        if (this.pos.x + wd > this.axis.x.x1 && this.thumbs.right.x - this.pos.x - offset > lmt) {
          this.thumbs.left.x = this.pos.x - offset; //this.thumbs.left.y = this.ds.height -this.scl*40
      ...
      return "".concat(this.thumbs.left.x, " ,  ").concat(this.thumbs.left.y);
    },
    thumbYY: function thumbYY() {
      var wd = this.scl * 31,
          lmt = this.thumbs.step * this.limitSize; //let pwbox = this.priceDigits*this.fs 
      ...
      if (this.draggingCenter && !this.rightDrug && !this.leftDrug && this.moveDrug) {
        ...
        if (xp <= this.axis.x.x2 && lx >= this.axis.x.x1 - wd) {
          ...
          this.thumbs.right.x = xp;
          this.wline.right.x1 = xp + wd > this.axis.x.x2 ? this.axis.x.x2 : xp + wd; //this.wline.middle.w = this.thumbs.right.x - this.thumbs.left.x
        ...
        if (this.pos.x < this.axis.x.x2 + wd && this.pos.x - offset - this.thumbs.left.x > lmt) {
          this.thumbs.right.x = this.pos.x - offset; //	this.thumbs.right.y = this.ds.height -this.scl*40
      ...
      return "".concat(this.thumbs.right.x, " ,  ").concat(this.thumbs.right.y);
    },
    bv: function bv() {
      //console.log("this.$refs.field  ", this.$refs.field)
      return this.$refs.chartSheet;
    },
    viewBoxSet: function viewBoxSet() {
      return "0 0 ".concat(this.ds.width, " ").concat(this.ds.height);
    },
    pointsAsPolyline: function pointsAsPolyline() {
      //return this.points.map((p, i) => `${i * 70} ${p.price}`).join(' ');
      return this.pointYX.map(function (p, i) {
        return "".concat(p.x, " ").concat(p.y);
      }).join(' ');
    },
    rects: function rects() {
      var rect = this.$refs.field ? this.$refs.field.getBoundingClientRect() : {
        width: this.ds.width,
        height: this.ds.height
      };
      return {
        rWidth: rect.width,
        rHeight: rect.height
    ...
    axis: function axis() {
      var off = this.ds.height * this.off / 100;
      var wbox = this.calcOffsetY();
      var lfBox = this.calcOffsetX(); //if (wbox < this.scl*31)  wbox = this.scl*31;

      var hbox = this.calcMiniBottomHight();
      var tbox = this.calcMiniTopHight();
      var x = off < lfBox / 3 ? lfBox / 3 : off;
      var y = off < hbox ? hbox : off;
      var y2 = tbox > off ? tbox : off; // let wbox = this.thumbs.right.priceDigits*this.fs -- this bad work

      var x2 = off < wbox / 1.5 ? wbox / 1.5 : off; //let west =(wbox/2 > )
    ...
    grid: function grid() {
      var off = this.ds.height * this.off / 100;
      var step = this.ds.height * this.tky / 100; //console.log(" step " , step);
  ...
  methods: {
    initSlider: function initSlider() {
      this.pos.x = this.axis.x.x1;
      var hf = (this.ds.height - (this.axis.y.y1 + this.fs * 1.18)) / 2 + this.scl * 40 / 2;
      var y = this.ds.height - hf; //console.log( ' init this.axis.x.x2 ',this.axis.x.x2 )
      ...
      this.wline.right.sSize = this.scl * 40 * 0.6; //console.log( this.ds.height -this.scl*40, "YYYYYYYYYYYYYYYY ",this.ds.height-hf,' this.axis.x.x2 ',this.axis.x.x2 )
    ...
    mousemove: function mousemove(e) {
      ...
      e.preventDefault();
      this.pos = this.getMousePosition(e); //if ( !this.leftDrug) this.pos = this.getMousePosition(e);
      //console.log( this.pos, " -x-y", this.leftDrug);
    },
    xmapData: function xmapData() {
      //let arr = bank.getData('mins')
      var sz = this.points.length > 0 ? this.points[0].data.length : 0;
      var step = (this.axis.x.x2 - this.axis.x.x1) / (sz - 1);
      this.thumbs.step = step; // this need for slider to display last point easy

      var i = this.axis.x.x1,
          aa = 0.00;
      ...
      if (this.points.length > 0) {
        this.points[0].data.forEach(function (a, j) {
          a.x = j == 0 ? i : i = step + i;
          a.id = j;
          aa = a.x;
        });
    ...
    getDisplayData: function getDisplayData(first, end) {
      ...
      this.pointsID.x2 = arr.length > 0 ? arr.slice(-1)[0].id : this.pointsID.x2; //console.log("this.pointsID",this.pointsID)
      ...
      if (this.pSize != arr.length) {
        ...
        this.ticksY = doAxis.ticksY().map(function (a) {
          return a;
        });
        this.ticksX.splice(0, this.ticksX.length);
        this.ticksX = doAxis.ticksX().map(function (a) {
          return a;
        });
    ...
    formatTicksX: function formatTicksX() {
      var _this3 = this;

      var prevDtm = this.ticksX.length > 0 ? this.ticksX[0].dtm : null;
      this.ticksX.forEach(function (a, i) {
        if (_this3.timeFotmat == 'HH:MM' || _this3.timeFotmat == 'HH:MM:ss') {
        ...
        prevDtm = a.dtm; //console.log(" prevDate ",prevDtm, a)
      });
    },
    reSize: function reSize(e) {
      // dev
    ...
    calcOffsetY: function calcOffsetY() {
      var f = this.decimals.length < 2 ? 2 : this.decimals.length;
      var maxInteger = this.points.length > 0 ? Math.max.apply(null, this.points[0].data.map(function (a) {
        return ('' + a.price).indexOf(".");
      })) : 1;
      if (maxInteger < 0) maxInteger = Math.max.apply(null, this.points[0].data.map(function (a) {
        return ('' + a.price).length;
      }));
      var mxl = maxInteger + f - 1;
      if (this.points.length == 0) mxl = 6;
      var boxWidth = mxl * this.fs + this.tsz.off;
      console.log(maxInteger, "=maxInteger", "f=", f, "mxl ", mxl);
      return boxWidth;
    },
    calcOffsetX: function calcOffsetX() {
      var mxl = this.timeFotmat ? this.timeFotmat.length : 0;
      if (mxl == 0) mxl = this.points.length > 0 ? Math.max.apply(null, this.points[0].data.map(function (a) {
        return ('' + a.tm).length;
      })) : 6;
      if (this.timeFotmat && mxl < 3) mxl = 3;
      var boxWidth = mxl * this.fs; //console.log( "mxl " , mxl, boxWidth, " -boxWidth  calcOffsetX()")

      return boxWidth;
    },
    calcMiniBottomHight: function calcMiniBottomHight() {
      var add = this.tsz.size + this.tsz.off;
      var miniHeight = this.fs * 1.18 + this.scl * 40 + this.ds.height / 100 * 3 + add;
      return miniHeight;
    },
    calcMiniTopHight: function calcMiniTopHight() {
      var miniHeight = this.fs * 1.18 + this.tsz.off;
      return miniHeight;
    },
    clearTest: function clearTest() {
      ...
      var wh = this.$refs.titles.getBoundingClientRect();
      var el = document.getElementById("legend");
      console.log('title box width', wh.width, " height ", wh.height);
      var rwh = this.ds.width / this.ds.height;
    ...
    loadChart: function loadChart() {
      ...
      // this.fillData(arr)
      this.xmapData(); //this.pSize=1;

      console.log(this.showGrid, " weChart  data ");
      var arr = this.points.length > 0 ? this.points[0].data.filter(function (a) {
        return a.id >= _this4.pointsID.x1 && a.id <= _this4.pointsID.x2;
      }) : []; //console.log( "loadChart() arr", arr.length, "x1=")

      var x1 = arr.length > 0 ? arr[0].x : this.axis.x.x1;
...
// EXTERNAL MODULE: ./src/components/weChart.vue?vue&type=style&index=0&id=70a9b30a&scoped=true&lang=stylus&
var weChartvue_type_style_index_0_id_70a9b30a_scoped_true_lang_stylus_ = __webpack_require__("e167");
...
//# sourceMappingURL=WeChart.umd.js.map