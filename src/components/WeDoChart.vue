...
const p = defineProps({
  ...
  ,shapes: { type:Array, default: () =>( new Array()) }
...
const pointsAsTriangel= (x,y,typ,sz)=>{
  let out=`${x},${y} ${x+sz},${y+sz} ${x-sz},${y+sz} ${x},${y}`
  ,size=(sz)?sz:p.fs/5;
  switch (typ)
    {
      case "dwtrg":
        out= `${x},${y} ${x+size},${y+size} ${x-size},${y+size} ${x},${y}`
        break;
      case "uptrg":
        out= `${x},${y} ${x+size},${y-size} ${x-size},${y-size} ${x},${y}`
        break;
...
svg#sheet(ref="sheet" :viewBox="viewBoxSet" xmlns="http://www.w3.org/2000/svg" @mousedown="slider.startDrag($event)" @mousemove="mousemove($event,pos)" @wheel="zoom" @mouseleave="slider.stopDrag" data-testid="sheet")
  ...
  line.axisY(:x1="axis.y.x1" :x2="axis.y.x2" :y1="axis.y.y1" :y2="axis.y.y2" data-testid="axis-Y")
  line.axisX(:x1="axis.x.x1" :x2="axis.x.x2" :y1="axis.x.y1" :y2="axis.x.y2" data-testid="axis-X")
  g(v-if="!cross.hide")
