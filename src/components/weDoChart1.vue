...
<template lang="pug">
	svg#sheet(ref="sheet" :viewBox="viewBoxSet" xmlns="http://www.w3.org/2000/svg" @mousedown="startDrag" @mousemove="mousemove" @wheel="zoom")
		...
		g.leftThumb(@mouseleave="stopThumb()" :transform="'translate('+thumbs.left.x+','+thumbs.left.y+') scale('+scl+')'" @mouseenter="leftDrug = true" @mouseup="stopDrag")
...
export default {
	...
	methods: {
		...
		crossMove(){
			if (this.pos.x<this.axis.x.x2 && this.pos.x>this.axis.x.x1 
				   && this.pos.y>this.axis.y.y2  && this.pos.y<this.axis.y.y1 ) {
